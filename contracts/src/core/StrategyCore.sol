...
contract StrategyCore is Ownable, Pausable, ReentrancyGuard {
    ...
    function updatePenguPrice(uint256 newPriceWei) external onlyCoordinatorOrOwner {
        ...
        if (newPriceWei > 1000 ether)
            revert PriceOutOfBounds(newPriceWei, 0, 1000 ether);
    ...
    function getOraclePriceInfo()
        external
        view
        returns (uint256 price, uint256 lastUpdated, bool isFresh, uint256 age)
    {
    ...
    function depositPengu(
        uint256 amountPengu,
        uint256 ethSpent
    ) external onlyFeeCollector whenNotPaused nonReentrant {
        ...
        uint256 allowanceAmount = penguToken.allowance(
            msg.sender,
            address(this)
        );
        ...
        bool success = penguToken.transferFrom(
            msg.sender,
            address(this),
            amountPengu
        );
     ...
     */
    function checkAndExecuteTP(
        uint256 lotId
    ) external validLotId(lotId) whenNotPaused nonReentrant tpEnabled {
     ...
     */
    function batchProcessTP(
        uint256 maxLots
    ) external whenNotPaused nonReentrant tpEnabled {
        ...
        while (
            processed < maxLots && currentLot < lotTail && activeLotCount > 0
        ) {
     ...
     */
    function checkAndExecuteTPForLots(
    ...
    function _processTPLadder(
        ...
        for (uint i = 0; i < tpLadder.length; i++) {
            ...
            uint256 targetPrice = (lot.avgPriceWeiPerPengu *
                tp.multiplierX1000) / 1000;

            if (currentPrice >= targetPrice && lot.amountPengu > 0) {
                // Calculate amount to sell for this TP level
                uint256 amountToSell = (originalAmount * tp.percentBps) /
                    BPS_DENOMINATOR;
    ...
    function _sellPengu(uint256 amount) internal returns (uint256 ethReceived) {
            ...
            )
        {
            ...
            if (ethReceived > 0) {
                try
                    IBuybackManager(buybackManager).receiveFromStrategy{
                        value: ethReceived
                    }(msg.sender)
                {
    ...
    function getLot(
        uint256 lotId
    ) external view validLotId(lotId) returns (Lot memory) {
        return lots[lotId];
    }
    ...
    function getAllActiveLots()
        external
        view
        returns (uint256[] memory lotIds, Lot[] memory activeLots)
    {
     ...
     */
    function emergencyDeactivateLot(
        uint256 lotId
    ) external onlyOwner validLotId(lotId) {
        Lot storage lot = lots[lotId];
        if (lot.active) {
            lot.active = false;
            activeLotCount--;
