...
contract Treasury is Ownable, Pausable, ReentrancyGuard {
    ...
    constructor(address _penguToken, address _initialOwner) Ownable(_initialOwner) {
    ...
    function finalizeEpoch(
        ...
        require(totalClaimable <= epoch.totalDividends, "Invalid claimable amount");
    ...
    function addDividends(uint256 amount) external onlyAuthorized whenNotPaused {
        require(currentEpoch > 0, "No active epoch");
        require(epochs[currentEpoch].endTime == 0, "Epoch already ended");
        require(amount > 0, "Amount must be positive");
        require(PENGU.transferFrom(msg.sender, address(this), amount), "Token transfer failed");
        epochs[currentEpoch].totalDividends += amount;
        emit DividendsAdded(currentEpoch, amount);
    }

    function emergencyAddDividends(uint256 epochId, uint256 amount)
        external onlyOwner validEpoch(epochId)
    {
    ...
    function hasUserClaimed(address user, uint256 epochId)
        external view validEpoch(epochId) returns (bool)
    {
        return userClaims[epochId][user].hasClaimed;
    }

    function getUserClaimInfo(address user, uint256 epochId)
    ...
    function isEpochClaimable(uint256 epochId) external view validEpoch(epochId) returns (bool) {
    ...
    function getCurrentEpochStatus()
        external view
        returns (
            uint256 activeEpochId, uint256 claimableEpochId,
            uint256 activeEpochStartTime, uint256 activeEpochDividends,
            uint256 claimableDividends, uint256 claimedAmount
        )
    {
        activeEpochId = currentEpoch;
        claimableEpochId = (currentEpoch > 1 && epochs[currentEpoch - 1].isClaimable)
            ? currentEpoch - 1 : 0;
        if (activeEpochId > 0) {
        ...
        validEpoch(epochId) 
    {
        ...
        require(epochId < currentEpoch - 1, "Cannot withdraw from recent epochs");
        require(amount <= epochs[epochId].totalDividends - epochs[epochId].claimedAmount, "Insufficient unclaimed amount");
