...
contract BuybackManager is Ownable, Pausable, ReentrancyGuard {
    ...
    event StrategyCoreUpdated(
        address indexed oldStrategyCore,
        address indexed newStrategyCore
    );
    ...
    constructor(
        ...
        Ownable(_owner)
        validAddress(_stratTokenAddress)
        validAddress(_strategyCore)
    ...
    function receiveFromStrategy(
        address caller
    ) external payable onlyStrategyCore whenNotPaused nonReentrant {
    ...
    function _executeBuyback(address caller) internal {
        ...
        try this._performSwapAndBurn(ethForBuyback) returns (
            uint256 stratBurned
        ) {
    ...
    function _performSwapAndBurn(
        uint256 ethAmount
    ) external returns (uint256 stratBurned) {
        ...
        uint256 minSTRAT = (expectedSTRAT * (BPS_DENOMINATOR - slippageBps)) /
            BPS_DENOMINATOR;

        // Record STRAT balance before swap
        uint256 stratBalanceBefore = IERC20(stratTokenAddress).balanceOf(
            address(this)
        );
        ...
        try IBurnableToken(stratTokenAddress).burn(stratReceived) {
            ...
            bool success = IERC20(stratTokenAddress).transfer(
                address(0xdead),
                stratReceived
            );
     ...
     */
    function getExpectedSTRAT(
        uint256 ethAmount
    ) external view returns (uint256 expectedSTRAT, uint256 minSTRAT) {
        ...
        try router.getAmountsOut(ethAmount, path) returns (
            ...
            minSTRAT =
                (expectedSTRAT * (BPS_DENOMINATOR - slippageBps)) /
                BPS_DENOMINATOR;
        } catch {
            return (0, 0);
        }
    ...
    function setBuybackThreshold(uint256 newThreshold) external onlyOwner {
        if (
            newThreshold < MIN_BUYBACK_THRESHOLD ||
            newThreshold > MAX_BUYBACK_THRESHOLD
        ) {
            revert ThresholdOutOfBounds(
                newThreshold,
                MIN_BUYBACK_THRESHOLD,
                MAX_BUYBACK_THRESHOLD
            );
        }
    ...
    function emergencyWithdrawETH(uint256 amount) external onlyOwner {
        ...
        if (withdrawAmount > 0) {
            uint256 pendingToReset = withdrawAmount > pendingETH
                ? pendingETH
                : withdrawAmount;
